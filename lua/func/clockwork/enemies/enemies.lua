...
function EXILE_M:ShroudDamage(entity)
	...
	Isaac.Spawn(EntityType.ENTITY_EFFECT,15,0,entity.Position, Vector(0,0), entity)
	sound:Play(SoundEffect.SOUND_FIREDEATH_HISS,1.3,0,false,1)
...
function EXILE_M:PeriwinkleUpdate(entity)
	...
	if not entity.Pathfinder:HasPathToPos(target.Position) or entity:CollidesWithGrid() then
	entity.Velocity = (target.Position - entity.Position):Normalized() * 1 * 7.8
	else
	entity.Pathfinder:FindGridPath(target.Position, 0.8, 1, true)
	end
	
	if entity:IsFrame(14, 0) then
	entity.Velocity = (target.Position - entity.Position):Normalized() * 1 * 7.8
	end
	...
	entity:AddVelocity((Isaac.GetFreeNearPosition(entity.Position, 300) - entity.Position):Normalized() * 7.5)
	entity.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_GROUND
	else
	entity.Velocity = (target.Position - entity.Position):Normalized() * 1 * 7.8
	entity.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_GROUND
...
GLASSPANEL = StageAPI.CustomGrid("Glass Panel", GridEntityType.GRID_PIT, nil, "gfx/periwinkle_glass.anm2", "Idle")


StageAPI.AddCallback("CaptainsLog", "POST_SPAWN_CUSTOM_GRID", 1, function(grindex, force, reSpawning, grid, persistentData)
	if persistentData.GlassState == nil then persistentData.GlassState = 0 end
	if persistentData.Spawn == nil then persistentData.Spawn = false end
end, "Glass Panel")

StageAPI.AddCallback("CaptainsLog", "POST_CUSTOM_GRID_UPDATE", 1, function(grid, spawnIndex, persistentData, CustomGrid, customGridTypeName)
	local room = Game():GetRoom()
	local player = Isaac.GetPlayer(0)
	local sprite = grid.Sprite
	if not sound:IsPlaying(Sounds.INSECTSWARMDUMMY) and persistentData.Spawn == false then
	sound:Play(Sounds.INSECTSWARMDUMMY,1,0,false,1)

	if persistentData.GlassState == 0 then
	if not sprite:IsPlaying("Idle") then
	sprite:Play("Idle", false)
	persistentData.Spawn = true
	end
	if persistentData.Spawn == true then
	persistentData.GlassState = 2
	end
	end

	if persistentData.GlassState == 2 then
	if not sprite:IsPlaying("Break") then
	sprite:Play("Break", false)
		Game():ShakeScreen(8,8)
		--sound:Play(Sounds.SPLASH_WATER,1,0,false,1)
		sound:Play(Sounds.GLASSBREAK, 0.90, 0, false, 1)
		Isaac.Spawn(EntityType.PERIWINKLE, Isaac.GetEntityVariantByName("Periwinkle"), 0, grid.Position, Vector(0,0), nil)
	end
	if sprite:IsEventTriggered("spawn") then

	end
	if sprite:IsFinished("Break") then
	persistentData.GlassState = 3
	end
	end
	
	if persistentData.GlassState == 3 then
	if not sprite:IsPlaying("IdleBroken") then
	sprite:Play("IdleBroken", false)
	end
