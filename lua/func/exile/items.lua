...
function EXILE_M.getRandomEnemyArtemis(vulnerable, ignoreFriendly, activeenemy)
	local room = Game():GetRoom()
	local entitylist = room:GetEntities()
    ...
    repeat
      targ = EXILE_M.randomFrom(Isaac.GetRoomEntities())
      i = i + 1
    until ( (not vulnerable or targ:IsVulnerableEnemy()) and (not targ:GetData().IgnoreArtemis) and (not activeenemy or targ:IsActiveEnemy()) and not (ignoreFriendly and targ:HasEntityFlags(EntityFlag.FLAG_FRIENDLY)) ) or i == 100
...
function EXILE_M:SwordUpdate(swordEntity)
  ...
  if player:HasCollectible(CollectibleType.COLLECTIBLE_DEATHS_TOUCH) then
   ...
   swordPositionOffset = swordEntity.PositionOffset
	if not sprite:IsPlaying("SwingDown") and not sprite:IsPlaying("SwingUp") and not sprite:IsPlaying("SwingLeft") and not sprite:IsPlaying("SwingRight") and player:GetFireDirection() == 2 then
   ...
   if not sprite:IsPlaying("SwingDown") and not sprite:IsPlaying("SwingUp") and not sprite:IsPlaying("SwingLeft") and not sprite:IsPlaying("SwingRight") and player:GetFireDirection() == 0 then
   ...
   if not sprite:IsPlaying("SwingDown") and not sprite:IsPlaying("SwingUp") and not sprite:IsPlaying("SwingLeft") and not sprite:IsPlaying("SwingRight") and player:GetFireDirection() == 1 then
   ...
   if not sprite:IsPlaying("SwingDown") and not sprite:IsPlaying("SwingUp") and not sprite:IsPlaying("SwingLeft") and not sprite:IsPlaying("SwingRight") and player:GetFireDirection() == 3 then
...
EXILE_M:AddCallback(ModCallbacks.MC_NPC_UPDATE, function(_, entity)
  ...
  if data.ArtemisLinkEffect then
		...
		EXILE_M.getRandomEnemyArtemis(true, true, true)
  ...
  if data.ArtemisLinkEffectTarg and not data.ArtemisLinkEffect then
    entity:SetColor(artemisColor, 2, 5, true, true)
		if entity:IsDead() then
		data.ArtemisLinkEffectTarg = false
		data.ArtemisLinkParent = nil
		end
  end
